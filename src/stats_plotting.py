...
def build_plotly_template(colors: dict[str, str], sequence: list[str]) -> go.layout.Template:
    ...
    return go.layout.Template(
        layout=go.Layout(
            ...
            colorway=sequence,
            xaxis=dict(gridcolor=colors['secondary'], zerolinecolor=colors['secondary'],
                       title_font=dict(size=12), automargin=True),
            yaxis=dict(gridcolor=colors['secondary'], zerolinecolor=colors['secondary'],
                       title_font=dict(size=12), automargin=True),
...
def plot_hours_per_project(hours: pl.DataFrame | None, user: str) -> go.Figure:
    ...
    fig = go.Figure(data=[go.Pie(labels=user_data["project"].to_list(), values=user_data["total_hours"].to_list(),
                    ...
                    layout=go.Layout(title=f'Stunden pro Projekt \u2014 {user}', title_font=dict(size=18, color=_colors['text'], family='Arial, sans-serif'),
...
def plot_total_hours_per_user(total_hours: pl.DataFrame | None, date_range: str) -> go.Figure:
    ...
    fig = go.Figure(data=[go.Bar(x=total_hours["user"].to_list(), y=total_hours["total_hours"].to_list(), marker_color=_colors['accent'])],
...
def plot_average_hours_per_user(average_hours: pl.DataFrame | None) -> go.Figure:
    ...
    fig =  go.Figure(data=[go.Bar(x=average_hours["user"].to_list(), y=average_hours["average_hours"].to_list(), marker_color=_sequence[1]),],
...
def plot_average_hours_per_period(average_hours: pl.DataFrame | None, period_days: int) -> go.Figure:
    ...
    fig = go.Figure(data=[go.Bar(x=average_hours["user"].to_list(), y=average_hours["average_hours"].to_list(), marker_color=_sequence[0]),],
...
def plot_time_series_analysis(daily_df: pl.DataFrame | None, weekly_avg: pl.DataFrame | None, weekday_avg: pl.DataFrame | None) -> tuple[go.Figure, go.Figure, go.Figure]:
...
def plot_cluster_analysis(features_df: pl.DataFrame | None, cluster_profiles: list[dict]) -> tuple[go.Figure, go.Figure]:
